...
(function () {
  ...
  async function loadModuleFromUrl(url) {
    ...
    try {
      ...
      // SỬA: clone modal sang body và xóa bản gốc trong moduleContent
      modals.forEach(m => {
        const clone = m.cloneNode(true);
        document.body.appendChild(clone);
        window._injectedModals.push(clone);
        m.remove(); // xóa bản gốc khỏi moduleContent, KHÔNG xóa clone
      });
      ...
      // BỎ đoạn remove lần 2 các modal vì đã remove bản gốc ở trên
