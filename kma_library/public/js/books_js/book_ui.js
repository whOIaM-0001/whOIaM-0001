...
(async function () {
  ...
  // Elements
  const modalEl = document.getElementById('modal') || document.getElementById('modal-book');
  const modalFormEl = document.getElementById('modal-form') || document.getElementById('modal-form-book');
  const btnModalClose = document.getElementById('modal-close') || document.getElementById('modal-close-book');
  const btnModalCancel = document.getElementById('modal-cancel') || document.getElementById('modal-cancel-book');
  ...
  // This fetches data from the backend DB endpoints and fills
  // <select id="MaTheLoai"> and <select id="MaNhaXuatBan"> if present.
  async function populateSelects() {
    try {
      ...
      // remember current values (when editing)
  ...
  try { if (modalEl) modalEl.style.pointerEvents = (modalEl.getAttribute('aria-hidden') === 'true') ? 'none' : 'auto'; } catch(e){}

  // modal helpers
  function closeModal() {
    try {
      // blur any focused element inside modal first to avoid accessibility warnings
      if (modalFormEl && modalFormEl.contains(document.activeElement)) {
        try { document.activeElement.blur(); } catch(e){}
      }
    } catch(e){}
    try { if (modalEl) { modalEl.setAttribute('aria-hidden','true'); modalEl.style.pointerEvents='none'; } } catch(e){}
  }

  function openModal(title = 'Thêm sách mới', opts = {reset:true}) {
    if (!modalEl || !modalFormEl) return;
    if (opts.reset) { try { modalFormEl.reset(); delete modalFormEl.dataset.editing; delete modalFormEl.dataset.editId; } catch(e){} }
    populateSelects().catch(()=>{}).then(()=> {
      const titleEl = document.getElementById('modal-title') || document.getElementById('modal-title-book'); if (titleEl) titleEl.textContent = title;
      modalEl.setAttribute('aria-hidden','false'); modalEl.style.pointerEvents='auto';
      setTimeout(()=> { const firstInput = modalFormEl.querySelector('input,select,textarea,button'); if (firstInput) firstInput.focus(); }, 10);
    });
  }
  ...
  function openEditModal(tr) {
    ...
    fields.forEach((f,i)=> { if (inputs[f]) inputs[f].value = (tr.children[i] && tr.children[i].textContent)? tr.children[i].textContent.trim() : ''; });
    modalFormEl.dataset.editing = 'true';
    modalFormEl.dataset.editId = (tr.children[0] && tr.children[0].textContent) ? tr.children[0].textContent.trim() : '';
    populateSelects().then(()=> openModal('Sửa sách', {reset:false})).catch(()=> openModal('Sửa sách', {reset:false}));
  }
  ...
  if (!document.__booksOutsideClick) {
    document.__booksOutsideClick = (ev) => {
      const t = ev.target;
      if (!t.closest('#section-books .action-menu') && !t.closest('#section-books .action-btn')) {
        closeAllActionMenus();
      }
    };
  ...
  // Note: sidebar/profile/menu code moved to `index.js` for global UI handling.
  // This file focuses on books module only; export menu handling remains below.
  ...
  if (exportMenu) {
    ...
    btnExportNodes.forEach(btn => btn.addEventListener('click', (e) => {
      ...
      // toggle display; ensure focused element is not left hidden
  ...
  // outside click to close action menus (module-safe)
  if (!document.__booksOutsideClick) {
    document.__booksOutsideClick = (ev) => {
      const t = ev.target;
      if (!t.closest('#section-books .action-menu') && !t.closest('#section-books .action-btn')) {
        try { window.closeAllActionMenus?.(); } catch(e){}
      }
    };
    document.addEventListener('click', document.__booksOutsideClick);
  }
