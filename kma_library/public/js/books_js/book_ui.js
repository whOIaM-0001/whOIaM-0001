...
(async function () {
  ...
  // Fallback confirm: dùng showConfirm nếu có, ngược lại dùng window.confirm
  ...
  async function populateSelects() {
    try {
      ...
      const fetches = [];
  if (selCat) fetches.push(fetch(`${DB_PATH}/categories.php`, { credentials: 'include' }).then(r => r.json()).catch(() => ({ ok: false })));
      else fetches.push(Promise.resolve(null));
  if (selPub) fetches.push(fetch(`${DB_PATH}/publishers.php`, { credentials: 'include' }).then(r => r.json()).catch(() => ({ ok: false })));
      else fetches.push(Promise.resolve(null));
  ...
  // build payload from form (unchanged)
  ...
  function bindModalEvents() {
    ...
    if (modalFormEl) {
      modalFormEl.onsubmit = onSubmit;
    }
  }
  bindModalEvents();

  // table utils: sorting/search/pagination (unchanged)
    ...
    if (!menu.__boundMenu) {
      menu.addEventListener('click', async (e) => {
        ...
        if (t.classList.contains('action-delete')) {
          ...
          try {
            const ok = await confirmDialog('Xác nhận xóa', `Xóa sách ${id} ?`, 'Xóa', 'Hủy');
            if (!ok) return;
  ...
  if (exportMenu) {
    ...
    exportMenu.querySelectorAll('button').forEach(b => b.addEventListener('click', async (e) => {
      ...
      try {
        ...
        // Luôn tính lại TìnhTrang theo SoLuong trước khi export
        const data = (res.data || []).map(r => ({ ...r, TinhTrang: toInt(r?.SoLuong) > 0 ? 'Còn' : 'Hết' }));
  ...
  async function loadAll() {
    try {
      ...
      // Luôn tính lại TìnhTrang theo SoLuong trước khi render bảng
      allRows = (res.data || []).map(r => ({ ...r, TinhTrang: toInt(r?.SoLuong) > 0 ? 'Còn' : 'Hết' }));
      currentPage = 1;
      updateRender();
      bindHeaderSort();
    } catch (err) {
