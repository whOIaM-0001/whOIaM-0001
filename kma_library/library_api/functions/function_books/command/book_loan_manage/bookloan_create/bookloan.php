<?php
// bookloan.php — Quản lý phiếu mượn (quanlymuon) + hạch toán tồn kho
...
// Kho sách: giảm/increase an toàn, tránh âm kho (không cần FOR UPDATE)
...
if ($method === 'GET') {
  if ($id) {
    ...
    // Chỉ tự tính lại nếu chưa "Đã trả"
...
if ($method === 'PUT' && $action === 'return') {
  ...
  try {
    ...
    if ($row['TinhTrang'] !== 'Đã trả') {
      ...
      // tính số ngày quá hạn tại thời điểm trả (nếu có)
      $today = todayYMD();
      $overdue = 0;
...
if ($method === 'POST') {
  ...
  try {
    ...
    // tính tình trạng ban đầu (chưa trả)
    $today = todayYMD();
...
if ($method === 'PUT' && $action === '') {
  ...
  try {
    ...
    // Nếu đổi sách hoặc đổi số lượng -> điều chỉnh kho
    ...
    if ($newMaSach !== $oldMaSach) {
      ...
      // trừ kho mới theo số lượng mới
      reserve_stock($pdo, $newMaSach, $newSL);
    } else {
      $delta = $newSL - $oldSL;
      if ($delta > 0) {
        // mượn thêm
        reserve_stock($pdo, $newMaSach, $delta);
      } else if ($delta < 0) {
        // giảm số mượn -> hoàn kho phần giảm
        release_stock($pdo, $newMaSach, -$delta);
      }
    }

    // Thực hiện UPDATE trường
    ...
    // Nếu bản ghi chưa 'Đã trả' thì cập nhật tình trạng tự động
    if ($row['TinhTrang'] !== 'Đã trả') {
