...
try {
  ...
  if ($method === 'POST') {
    ...
    $soluong = isset($input['SoLuong']) ? max(0, (int)$input['SoLuong']) : 0;
    $tinhtrang = ($soluong > 0) ? 'Còn' : 'Hết'; // TỰ TÍNH — không lấy từ input
  ...
  if ($method === 'PUT') {
    ...
    // KHÔNG cho cập nhật trực tiếp TinhTrang từ client — chỉ tính theo SoLuong
    $fields = ['TenS','MaTheLoai','Tacgia','NamXB','MaNhaXuatBan','SoLuong'];
    $sets = []; $vals = [];

    // uniqueness check for TenS (nếu có)
    if (isset($input['TenS'])) {
    ...
    foreach ($fields as $f) {
      if (array_key_exists($f, $input)) {
        if ($f === 'SoLuong') {
          $val = max(0, (int)$input[$f]);
          $sets[] = "`$f` = ?"; $vals[] = $val;
          // Cập nhật kèm TinhTrang theo SoLuong mới
          $sets[] = "`TinhTrang` = ?"; $vals[] = ($val > 0 ? 'Còn' : 'Hết');
        } else {
          $sets[] = "`$f` = ?"; $vals[] = $input[$f];
        }
      }
